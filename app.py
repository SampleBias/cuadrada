...
def analyze_paper_with_agent(agent, filepath, reviewer_name, submission_id):
    ...
    try:
        print(f"Starting analysis for {reviewer_name} on file: {filepath}")
        
        # Check if file exists
        if not os.path.exists(filepath):
            raise FileNotFoundError(f"Paper file not found at: {filepath}")
        
        # Track original model and potential fallbacks
        original_model = agent.current_model
        print(f"{reviewer_name}: Using model {original_model}")
        ...
        if model_downgraded:
            print(f"{reviewer_name}: Model was downgraded from {original_model} to {final_model}")
        
        if not review_text:
            raise ValueError(f"No review text generated by {reviewer_name}")
        
        print(f"{reviewer_name}: Successfully generated review text ({len(review_text)} chars)")
        ...
        error_msg = f"File error for {reviewer_name}: {str(e)}"
        print(error_msg)
        import traceback
        print(f"Full traceback: {traceback.format_exc()}")
        return {
            'decision': 'ERROR',
            'summary': f"Unable to locate the paper file. Please try uploading again.",
            'full_review': f"File error: {str(e)}",
            'accepted': False,
        ...
        return {
            ...
            'full_review': f"API Error: {str(e)}",
            'accepted': False,
            'model_used': agent.current_model if hasattr(agent, 'current_model') else 'unknown',
            'error_type': 'APIError'
        }
    except Exception as e:
        error_msg = f"Unexpected error in analysis with {reviewer_name}: {str(e)}"
        print(error_msg)
                    ...
                    if submission_id in pickled_results:
                        ...
                        if result_data.get('processing_complete', False) or (result_data.get('results') and len(result_data.get('results', {})) >= 3):
                            ...
                            for reviewer, data in results_data.items():
                                decision = data.get('decision', 'NO_DECISION')
                                print(f"DEBUG: {reviewer}: {decision} - {type(data)}")
                                if decision == 'ERROR':
                                    print(f"DEBUG: {reviewer} ERROR DETAILS: {data.get('summary', 'No error message available')}")
